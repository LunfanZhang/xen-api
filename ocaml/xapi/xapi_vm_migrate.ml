...
(* Helper function to apply a 'with_x' function to a list.
   Each [withfn] invocation calls its continuation EXACTLY ONCE with a
   list of accumulated values for that item (e.g. a leaf mirror record
   plus any per-snapshot mirror records). Items are accumulated via [@]
   so [fn] sees a flat list. *)
