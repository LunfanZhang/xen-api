...
let migrate_send' ~__context ~vm ~dest ~live:_ ~vdi_map ~vdi_format_map ~vif_map
  ...
  try
    (* Sort VDIs by size in principle. This gives better chances that similar
       but smaller VDIs would arrive comparatively earlier, which can serve as
       base for incremental copying the larger ones. Same-sized VDIs are then
       ordered by their position in the snapshot trees. *)
    let position =
      (* A depth-first walk of the snapshot trees: a node comes after the
         one it is copied onto, and a branch finishes before its siblings
         start. Siblings go oldest first, so the disk itself comes last. *)
      let age v =
      ...
      let children_of vconf =
        ...
        |> List.sort (fun v1 v2 ->
            match compare (is_snapshot v2) (is_snapshot v1) with
            | 0 ->
                compare (age v1) (age v2)
            | r ->
                r
        )
      in
      let is_root vconf =
        match vconf.snapshot_parent with
        | None ->
            true
        | Some parent ->
            not (List.exists (fun v -> v.vdi = parent) all_vdis)
      in
      let index = Hashtbl.create (List.length all_vdis) in
      let next = ref 0 in
      let rec visit vconf =
        Hashtbl.replace index vconf.vdi !next ;
        incr next ;
        List.iter visit (children_of vconf)
      in
      List.iter visit (List.filter is_root all_vdis) ;
      fun vconf ->
        Hashtbl.find_opt index vconf.vdi |> Option.value ~default:max_int
    in
    let compare_fun v1 v2 =
      match Int64.compare v1.size v2.size with
      | 0 ->
          compare (position v1) (position v2)
      | r ->
          r
    in
