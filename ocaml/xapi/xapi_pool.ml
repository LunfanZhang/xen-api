...
let sync_repos ~__context ~self ~repos ~force ~token ~token_id =
  ...
  |> List.iter (fun repo ->
         ...
         let full_complete = sync ~__context ~self:repo ~token ~token_id in
         (* Dnf and our custom yum-utils sync all the metadata including updateinfo,
          * Thus no need to re-create pool repository *)
         if Pkgs.manager = Yum && full_complete = false then
